...
def get_shipped_addresses(file):
    """Extract normalized addresses from a shipment CSV/XLSX."""
    ...
    for col in df.columns:
        if 'address' in col.lower() or 'ship' in col.lower():
            addr_col = col
            break
    if not addr_col:
...
if st.session_state.step >= 2:
    ...
    col1, col2 = st.columns(2)
    with col1:
        old_file = st.file_uploader("Old Shipments", type=["csv", "xlsx"], key="old")
    with col2:
        recent_file = st.file_uploader("Recent Shipments", type=["csv", "xlsx"], key="recent")

    if old_file or recent_file:
        shipped = set()
        if old_file:
            shipped |= get_shipped_addresses(old_file)
        if recent_file:
        ...
        if new_rows:
            ...
            st.download_button("⬇️ Download Validated CSV", data=csv_bytes, file_name=filename, mime="text/csv")
        else:
